...
pub const Block3 = packed struct(u8) {
    ...
    pub fn execute(self: Block3, cpu: *Cpu) void {
        switch (self.z) {
            0x0 => @as(Block3Z0, @bitCast(self)).execute(cpu),
            0x1 => @as(StackRetOp, @bitCast(self)).execute(cpu),
            ...
            0x7 => x.execCall(cpu, @as(u16, self.y) << 3), // tgt * 8
        }
    }
};
    ...
    fn execute(self: Block3Z0, cpu: *Cpu) void {
        if (self.is_not_RET) {
            switch (self.opcode) {
                ...
                .LDH_A_n8 => {
                    const val: u8 = cpu.read8(@as(u16, cpu.pc_pop_8()) | 0xFF00);
                    cpu.AF.setHi(val);
                },
                .LD_HL_SP_plus_n8 => {
                    const offset: i8 = @bitCast(cpu.pc_pop_8());
    ...
    fn execute(self: JumpLoadOp, cpu: *Cpu) void {
        if (self.is_ld) {
            switch (self.opcode) {
                ...
                .LD_n16_A => cpu.write8(cpu.pc_pop_16(), cpu.AF.getHi()),
                .LDH_A_C => {
                    const addr: u16 = 0xFF00 | @as(u16, cpu.BC.getLo());
                    cpu.AF.setHi(cpu.read8(addr));
                },
                .LD_A_n16 => cpu.AF.setHi(cpu.read8(cpu.pc_pop_16())),
            }
        } else {
            const cond: u2 = @bitCast(self.opcode);
            const addr = cpu.pc_pop_16();
...
pub const AluImmOp = packed struct(u8) {
    ...
    pub fn execute(self: AluImmOp, cpu: *Cpu) void {
        switch (self.op) {
            .ADD => x.execAdd8(cpu, .a, Cpu.setR8, cpu.AF.getHi(), cpu.pc_pop_8(), false),
            .ADC => x.execAdd8(cpu, .a, Cpu.setR8, cpu.AF.getHi(), cpu.pc_pop_8(), true),
            .SUB => x.execSub8(cpu, .a, Cpu.setR8, cpu.AF.getHi(), cpu.pc_pop_8(), false),
            .SBC => x.execSub8(cpu, .a, Cpu.setR8, cpu.AF.getHi(), cpu.pc_pop_8(), true),
            .AND => x.execAnd(cpu, .a, Cpu.setR8, cpu.AF.getHi(), cpu.pc_pop_8()),
            .XOR => x.execXor(cpu, .a, Cpu.setR8, cpu.AF.getHi(), cpu.pc_pop_8()),
            .OR => x.execOr(cpu, .a, Cpu.setR8, cpu.AF.getHi(), cpu.pc_pop_8()),
